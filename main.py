...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    Use `matplotlib` to generate a figure of the distance & #expanded-nodes
     w.r.t. the weight.
    TODO [Ex.14]: Complete the implementation of this method. (Done)
    """
    ...
    p1 = ax1.plot(weights, total_cost, color='blue', linestyle='solid', label='Solution cost')[0]
    ...
    p2 = ax2.plot(weights, total_nr_expanded, color='red', linestyle='solid', label='#Expanded states')[0]
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    arr = np.linspace(0.5, 1, n)
    cost_list = []
    nr_expanded_states_list = []
    weight_list = []
    for weight in arr:
        ast = AStar(heuristic_type, weight, max_nr_states_to_expand)
    ...
    plot_distance_and_expanded_wrt_weight_figure(problem.name, weight_list, cost_list, nr_expanded_states_list)
...
def toy_map_problem_experiments():
    ...
    toy_map_problem = MapProblem(streets_map, 54, 549)
    ast = AStar(NullHeuristic)
    res1 = ast.solve_problem(toy_map_problem)
    print(res1)
    ...
    #       solve the same `toy_map_problem` with it and print the results (as before).
    toy_map_problem = MapProblem(streets_map, 54, 549)
    ast = AStar(AirDistHeuristic)
    res2 = ast.solve_problem(toy_map_problem)
    print(res2)
...
def deliveries_truck_problem_with_astar_experiments():
    ...
    ast = AStar(TruckDeliveriesMaxAirDistHeuristic)
    res = ast.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)

    # Ex.21
    # TODO: create an instance of `AStar` with the `TruckDeliveriesSumAirDistHeuristic`,
    #       solve the `moderate_delivery_problem_with_distance_cost` with it and print the results.
    ast = AStar(TruckDeliveriesSumAirDistHeuristic)
    res = ast.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)
    ...
    #       solve the `moderate_delivery_problem_with_distance_cost` with it and print the results.
    ast = AStar(TruckDeliveriesMSTAirDistHeuristic)
    res = ast.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)
...
def multiple_objectives_deliveries_truck_problem_experiments():
    ...
    ast = AStar(TruckDeliveriesMSTAirDistHeuristic)
    res = ast.solve_problem(small_delivery_problem_with_time_cost)
    print(res)
    ...
    ast = AStar(TruckDeliveriesMSTAirDistHeuristic)
    res = ast.solve_problem(small_delivery_problem_with_money_cost)
    print(res)
...
def deliveries_truck_problem_with_astar_epsilon_experiments():
    ...
    aste = AStarEpsilon(TruckDeliveriesMSTAirDistHeuristic, within_focal_h_sum_priority_function, 0.5, None, 0.03, 40)
    res = aste.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)
...
def deliveries_truck_problem_anytime_astar_experiments():
    ...
    any = AnytimeAStar(TruckDeliveriesMSTAirDistHeuristic, 50)
    res = any.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)
...
def big_deliveries_truck_problem_with_non_acceptable_heuristic_and_anytime_astar_experiments():
    ...
    any = AnytimeAStar(TruckDeliveriesSumAirDistHeuristic, 400)
    res = any.solve_problem(big_delivery_problem_with_distance_cost)
    print(res)

    any = AnytimeAStar(TruckDeliveriesMSTAirDistHeuristic, 400)
    res = any.solve_problem(big_delivery_problem_with_distance_cost)
    print(res)
