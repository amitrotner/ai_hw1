...
class TruckDeliveriesSumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        while len(all_junctions_in_remaining_truck_path) > 1:
            all_junctions_in_remaining_truck_path = all_junctions_in_remaining_truck_path-{current}
            # find the min distance from current and add to total_cost_of_greedily_built_path
            min_dist = min(
                self.cached_air_distance_calculator.get_air_distance_between_junctions(current, junction)
                for junction in all_junctions_in_remaining_truck_path)
            total_cost_of_greedily_built_path += min_dist
            # find the next current according the min distance
            for junction in all_junctions_in_remaining_truck_path:
                if self.cached_air_distance_calculator.get_air_distance_between_junctions(current,
                                                                                          junction) == min_dist:
                    current = junction
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: Set[Junction]) -> float:
        ...
        TODO [Ex.23]: Implement this method.
              Use `networkx` (nx) package (already imported in this file) to calculate the weight
               ...
               and there is an edge between each pair of distinct junctions (no self-loops) fo r which
        ...
        """
        g = nx.empty_graph(len(junctions))
        for junction1 in junctions:
            for junction2 in junctions-{junction1}:
                g.add_edge(junction1.index, junction2.index,
                           weight=self.cached_air_distance_calculator.get_air_distance_between_junctions(junction1,
                                                                                                         junction2))
