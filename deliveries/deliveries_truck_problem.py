...
class DeliveriesTruckState(GraphProblemState):
    ...
    def __eq__(self, other):
        ...
        # TODO [Ex.15]: Complete the implementation of this method! (Done)
        ...
        return (self.current_location == other.current_location and
                self.loaded_deliveries == other.loaded_deliveries and
    ...
    def get_total_nr_packages_loaded(self) -> int:
        ...
        TODO [Ex.15]: Implement this method. (Done)
...
class DeliveriesTruckProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        for delivery in self.get_deliveries_waiting_to_pick(state_to_expand):  # yielding the pick location
            if self.problem_input.delivery_truck.max_nr_loaded_packages >= \
                ...
                # check if there is enough space for the packages
                yield OperatorResult(DeliveriesTruckState(state_to_expand.loaded_deliveries | frozenset([delivery]),
                                                          state_to_expand.dropped_deliveries, delivery.pick_location),
                                     self.map_distance_finder.get_map_cost_between(state_to_expand.current_location,
                                                                                   delivery.pick_location),
                                     'pick ' + delivery.client_name)
        for delivery in state_to_expand.loaded_deliveries:  # yielding the drop location
            yield OperatorResult(
                DeliveriesTruckState(state_to_expand.loaded_deliveries - frozenset([delivery]),
                                     state_to_expand.dropped_deliveries | frozenset([delivery]),
                                     delivery.drop_location),
                self.map_distance_finder.get_map_cost_between(state_to_expand.current_location, delivery.drop_location),
                'drop ' + delivery.client_name)
    ...
    def _calc_map_road_cost(self, link: Link) -> DeliveryCost:
        """
        TODO [Ex.27]: Modify the implementation of this method, so that for a given link (road), it would return (Done)
        ...
        return DeliveryCost(
            ...
            time_cost=link.distance / optimal_velocity,  # TODO: modify this value!
            money_cost=((gas_cost_per_meter +
                         link.is_toll_road * self.problem_input.toll_road_cost_per_meter) * link.distance),
    ...
    def get_cost_lower_bound_from_distance_lower_bound(self, total_distance_lower_bound: float) -> float:
        ...
        TODO [Ex.28]: We left only partial implementation of this method (just the trivial distance objective). (Done)
    ...
    def get_deliveries_waiting_to_pick(self, state: DeliveriesTruckState) -> Set[Delivery]:
        ...
        """
        return (set(self.problem_input.deliveries) - state.loaded_deliveries) - (
            state.dropped_deliveries)
    ...
    def get_all_junctions_in_remaining_truck_path(self, state: DeliveriesTruckState) -> Set[Junction]:
        ...
        TODO [Ex.17]: Implement this method. (Done)
        ...
        """
        return {state.current_location}.union(
            {delivery.pick_location for delivery in self.get_deliveries_waiting_to_pick(state)}).union(
            {delivery.drop_location for delivery in state.loaded_deliveries}).union(
            {delivery.drop_location for delivery in self.get_deliveries_waiting_to_pick(state)})
